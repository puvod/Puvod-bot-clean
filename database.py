...
def init_db():
    ...
    # 1. Vytvoření základní tabulky guild_settings (pokud neexistuje)
    cursor.execute('''
    ...
    cursor.close()
    conn.close()
    print("🗄️ PostgreSQL Databáze byla úspěšně inicializována a zkontrolována.")
...
def set_counting_number(guild_id: int, number: int):
    """Natvrdo nastaví aktuální číslo a vynuluje posledního píšícího."""
    conn = get_db_connection()
    cursor = conn.cursor()
    g_id = str(guild_id)
    ...
    cursor.execute(
        "UPDATE guild_settings SET current_number = %s, last_user_id = NULL WHERE guild_id = %s",
        (number, g_id)
    )
...
def increment_user_count(guild_id: int, user_id: int):
    """Zvýší celkové skóre uživatele v leaderboardu o 1."""
    conn = get_db_connection()
    cursor = conn.cursor()
    g_id = str(guild_id)
    u_id = str(user_id)
...
def get_top_users(guild_id: int, limit=10):
    """Vrátí TOP x hráčů pro daný server."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT user_id, total_counts FROM counting_leaderboard WHERE guild_id = %s ORDER BY total_counts DESC LIMIT %s",
        (str(guild_id), limit)
    )
...
def get_all_guilds_with_counting():
    """Vrátí servery, které mají nastavený čas pro topku."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT guild_id, counting_channel_id, counting_time FROM guild_settings WHERE counting_channel_id IS NOT NULL AND counting_time IS NOT NULL"
    )
    results = cursor.fetchall()
    cursor.close()
    conn.close()
